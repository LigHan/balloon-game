...
В спокойном состоянии сцена занимает высоту всей страницы, а деревушка находится у её нижнего края. При запуске полёта слои сохраняют положение на экране, хотя сама сцена закрепляется в окне браузера. `web/sky-scene.js` ведёт движение в одном непрерывном цикле `requestAnimationFrame`, используя время начала и скорость роста из параметров раунда. Ответы сервера мягко уточняют время; округлённые значения коэффициента не задают отдельные шаги анимации. Слои перемещаются через `translate3d`, дальний слой движется медленнее ближнего. При потере связи движение плавно останавливается, а `prefers-reduced-motion` отключает его. Красный маршрут меняет небо на тёплое закатное.

Деревушка масштабируется по ширине с сохранением пропорций SVG (`height: auto`), чтобы рисунок заполнял нижний край и на широких мониторах. Минимальная высота страницы равна высоте окна; на высоких экранах футер и земля остаются внизу.
